    ...
    uint8 **pp_in_frame, uint16 *p_enc_frame_len)
{
    ...
    if (status == OS_SUCCESS)
    {
        ...
        memcpy(p_new_enc_frame, p_in_frame, TC_FRAME_PRIMARYHEADER_SIZE);
        ...
        */
        ...
        memcpy((p_new_enc_frame+index), (p_in_frame+TC_FRAME_PRIMARYHEADER_SIZE), tf_payload_len);
...
// Loads the ingest frame into the global tc_frame while performing decryption
{
    ...
    {   // Update last spi used
        ...
        {
            printf(KRED "Error: SCID incorrect! \n" RESET);
            status = OS_ERROR;
        }
        else
        {   
        ...
        {
            report.ispif = 1;
            printf(KRED "Error: SPI value greater than NUM_SA! \n" RESET);
            status = OS_ERROR;
        }
        if (status == OS_SUCCESS)
        {
        ...
        if (status == OS_SUCCESS)
        {
            ...
            {	
                printf(KRED "Error: SA invalid type! \n" RESET);
                status = OS_ERROR;
            }
        }
        ...
        //    {	
        //        printf(KRED "Error: VCID not mapped to provided SPI! \n" RESET);
        //        status = OS_ERROR;
        //    }
        //}
        if (status == OS_SUCCESS)
        {
            ...
            {
                printf(KRED "Error: SA state not operational! \n" RESET);
                status = OS_ERROR;
            }
        }
        if (status != OS_SUCCESS)
    ...
    {	// Authenticated Encryption
        #ifdef DEBUG
            printf(KBLU "ENCRYPTED TC Received!\n" RESET);
        #endif
        #ifdef TC_DEBUG
            printf("IV: \n");
        #endif
            ...
            sa_ptr->arcw[sa_ptr->arcw_len-1]) != OS_SUCCESS )
        {
            ...
            printf(KRED "Error: IV not in window! \n" RESET);
            #ifdef OCF_DEBUG
                Crypto_fsrPrint(&report);
            #endif
            status = OS_ERROR;
        ...
        else 
        {
            ...
            {   // Replay - IV value lower than expected
                ...
                printf(KRED "Error: IV replay! Value lower than expected! \n" RESET);
                #ifdef OCF_DEBUG
                    Crypto_fsrPrint(&report);
                #endif
                status = OS_ERROR;
        ...
        #ifdef DEBUG
            printf("Key ID = %d, 0x", sa_ptr->ekid);
            for(int y = 0; y < KEY_SIZE; y++)
            {
                printf("%02x", ek_ring[sa_ptr->ekid].value[y]);
            }
            printf("\n");
        #endif
        gcry_error = gcry_cipher_setkey(
        ...
        if((gcry_error & GPG_ERR_CODE_MASK) != GPG_ERR_NO_ERROR)
        {
            ...
            printf("Actual MAC   = 0x");
            for (int z = 0; z < MAC_SIZE; z++)
            {
                printf("%02x",tc_sdls_processed_frame->tc_sec_trailer.mac[z]);
            }
            printf("\n");
    ...
    #ifdef DEBUG
        printf(KYEL "----- Crypto_TC_ProcessSecurity END -----\n" RESET);
    #endif
...
// Accepts CCSDS message in ingest, and packs into TM before encryption
{
    ...
    // Initialize the temporary TM frame
        ...
        memcpy(tm_frame.tm_sec_header.iv, sa_ptr->iv, IV_SIZE);
